...
interface Product {
  ...
  addons: ExtraItem[]; // Adicionais
  opcionais?: ExtraItem[]; // Opcionais
  paused: boolean;
  maxFlavors?: number; // for especial
  prepTime?: string; // Tempo de preparo (ex: "30 min")
}
...
export default function App() {
              ...
              >
                ...
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-4">
                    ...
                    {product.category === "Pizzas" && product.prepTime && (
                      <p className="text-xs text-amber-400 mt-1 font-bold flex items-center gap-1 animate-pulse">
                        ⏱️ Preparo: {product.prepTime}
                      </p>
                    )}
                  </div>
                </div>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-fuchsia-500/40 bg-[#0d071d] p-6 shadow-[0_0_30px_rgba(217,70,239,0.2)]">
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                {selectedProduct.category === "Pizzas" && selectedProduct.prepTime && (
                  <p className="text-xs text-amber-400 font-bold mt-1">⏱️ Tempo de preparo: {selectedProduct.prepTime}</p>
                )}
              </div>
  ...
  const [newOpcionais, setNewOpcionais] = useState<ExtraItem[]>([]);
  const [newProductPrepTime, setNewProductPrepTime] = useState("");
  const [adminSearch, setAdminSearch] = useState("");
  const [adminStatusFilter, setAdminStatusFilter] = useState("todos");
  const [adminSortBy, setAdminSortBy] = useState("data");
  ...
  const openEditProduct = (prod: Product) => {
    ...
    setNewOpcionais(prod.opcionais || []);
    setNewProductPrepTime(prod.prepTime || "");
  };
  ...
  const handleSaveProduct = () => {
    ...
    const savedItem: Product = {
      ...
      addons: newAddons,
      opcionais: newOpcionais,
      paused: editingProduct ? editingProduct.paused : false,
      prepTime: newProductType === "meio-meio" || newProductCategory === "Pizzas" ? newProductPrepTime : undefined
    };
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-zinc-950/80 border border-fuchsia-500/30 p-6 rounded-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            {(newProductType === "meio-meio" || newProductCategory === "Pizzas") && (
              <input
                type="text"
                placeholder="Tempo de preparo (ex: 30-40 min)"
                value={newProductPrepTime}
                onChange={e => setNewProductPrepTime(e.target.value)}
                className="rounded-xl bg-black/40 border border-indigo-500/20 p-3 text-white"
              />
            )}
          </div>
          ...
          {/* OPCIONAIS (Meio-a-Meio e Especiais) */}
          {(newProductType === "meio-meio" || newProductType === "especial") && (
            <div className="mb-6 bg-violet-950/20 p-4 rounded-xl border border-violet-500/20">
              <label className="block text-sm font-semibold text-violet-400 mb-2">
                Opcionais (Ex: Sem cebola, Orégano extra - Itens selecionáveis para personalização)
              </label>
              <div className="space-y-3">
                {newOpcionais.map((opc, index) => (
                  <div key={index} className="flex gap-3 items-center">
                    <input
                      type="text"
                      value={opc.name}
                      onChange={e => {
                        const upd = [...newOpcionais];
                    ...
                    <button
                      ...
                      className={`p-2 rounded text-xs font-bold ${
                        opc.active ? "bg-green-600/40 text-green-300 border border-green-500" : "bg-red-600/40 text-red-300 border border-red-500"
                      }`}
                    >
                      {opc.active ? "Ativo" : "Pausado"}
                    </button>
                    <button
                      onClick={() => setNewOpcionais(newOpcionais.filter((_, i) => i !== index))}
                ...
                <button
                  onClick={() => setNewOpcionais([...newOpcionais, { id: crypto.randomUUID(), name: "Novo Opcional", price: 0.0, maxQty: 1, active: true }])}
                  className="flex items-center gap-2 text-sm text-violet-300 hover:text-white"
                >
        ...
        {/* Lista de Produtos Cadastrados com Filtros */}
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2 w-full md:w-auto">
              <input
                ...
                className="rounded-xl bg-black/40 border border-indigo-500/30 px-3 py-1.5 text-sm text-white w-full md:w-48"
              />
              <select
                value={adminStatusFilter}
                onChange={e => setAdminStatusFilter(e.target.value)}
                className="rounded-xl bg-black/40 border border-indigo-500/30 px-3 py-1.5 text-sm text-white"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {products
              ...
              .filter(p => {
                if (adminStatusFilter === "ativos") return !p.paused;
                if (adminStatusFilter === "pausados") return p.paused;
                return true;
              })
              .sort((a, b) => {
                if (adminSortBy === "maior_valor") return b.price - a.price;
                if (adminSortBy === "menor_valor") return a.price - b.price;
