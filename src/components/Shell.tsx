...
function routeHref(route: Route) {
  if (route === "site") return "./index.html#/site";
...
export function TopLinks({ current }: { current: Route }) {
  return (
    <div className="fixed top-3 right-3 z-50 glass-strong px-2 py-2 flex gap-1 text-xs">
      {(["site", "admin", "pdv"] as Route[]).map((route) => (
        <a
          key={route}
          href={routeHref(route)}
          style={current === route ? { background: "linear-gradient(135deg, var(--mh-btn), var(--mh-btn-end))", color: "var(--mh-btn-text)" } : undefined}
          className={"px-3 py-1.5 rounded-lg font-bold uppercase tracking-wider transition " +
            (current === route ? "shadow-lg" : "text-white/60 hover:text-white hover:bg-white/5")}
        >
          {route === "site" ? "🛒 Site" : route === "admin" ? "⚙️ Admin" : "💰 PDV"}
        </a>
      ))}
    </div>
  );
