        ...
        {lista.map(p => (
          <div key={p.id} className="card p-3 flex items-center gap-3">
            ...
            <div className="flex gap-1">
              ...
              <button className="btn-ghost !text-xs text-red-400" onClick={() => confirm("Excluir produto?") && setProdutos(produtos.filter(x => x.id !== p.id))}>🗑️</button>
            </div>
          </div>
        ))}
      </div>
...
function ProdutoEditor({ produto, onClose, onSave }: { produto: Produto; onClose: () => void; onSave: (p: Produto) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative glass-strong w-full max-w-3xl max-h-[92vh] overflow-auto p-5">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div className="md:col-span-2"><label className="text-xs text-white/60">URL da Imagem</label>
            <input className="input" value={p.imagens[0] || ""} onChange={e => setP({ ...p, imagens: [e.target.value] })} placeholder="https://..." />
          </div>
        ...
        <div className="flex gap-2 mt-5">
          ...
          <button className="btn-neon flex-1" onClick={() => onSave(p)}>💾 Salvar</button>
        </div>
      </div>
    </div>
  );
