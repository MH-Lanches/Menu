(() => {
  const STORAGE_OPEN = 'brasa_pdv_pedidos_v1';
  const STORAGE_EVENT = 'brasa_pdv_pedidos_evento_v1';
  ...
  async function pollRemoteOrders() {
    try {
      ...
      const data = await response.json();
      const incoming = parseOrdersFromPayload(data).map((raw) => ({ ...raw, origem: 'online' }));
  ...
  function bindRealtime() {
    window.addEventListener('storage', (event) => {
      if (event.key === STORAGE_OPEN || event.key === STORAGE_EVENT) syncFromStorage();
    });
